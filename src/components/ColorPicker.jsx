    ...
    <div onClick={()=> setIsOpen(!isOpen)}className='relative'>
        ...
        {isOpen && (
            <div className='grid grid-cols-4 w-60 gap-2 absolute top-full left-0 right-0 p-3 mt-2 z-10 bg-white rounded-md border border-gray-200 shadow-sm'>
                {colors.map((color)=>(
                    <div key={color.value} className='flex flex-col  relative cursor-pointer group'  onClick={()=> {onChange(color.value); setIsOpen(false)}}>
                        ...
                        <p className='text-x5 text-center mt-1 text-gray-600'>{color.name}</p>
